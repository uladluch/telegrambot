...
`/latest`, `/last5 N`, `/last10 N`, `/list`, `/add` (RSS/название/YouTube-канал),
`/del N`, `/video url`, `/audio url`, `/clear` (удаляет переписку с ботом, подписки
не трогает), `/help`. Первый `/start` фиксирует владельца, остальные игнорируются.
