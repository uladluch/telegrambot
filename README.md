...
(до 720p). Плюс разовые команды `/video` и `/audio` для любых YouTube-ссылок.
...
- `/last5 N` / `/last10 N` → N последних эпизодов подписки #N карточками.
...
- Кнопка → задание воркеру (`yt_video`/`yt_audio`/`send_audio`) → файл в чат.
  Видео до 720p H.264, до 2 ГБ. Shorts из лент исключаются.
- `/video <url>` / `/audio <url>` → разовое скачивание по ссылке.
- Файл >2 ГБ → карточка с прямой ссылкой.

## Команды бота

Интерфейс бота — английский. `/latest`, `/list` (подписки кнопками: тап → 5 последних),
`/last5 N`, `/last10 N` (+ кнопка «5 more»), `/add` (RSS/название/YouTube-канал),
...
(перевод Яндекса или родные). У русскоязычных каналов RU-ряда нет. Прогресс
скачивания — редактируемым статус-сообщением; в подписи файла — таймкоды глав.
